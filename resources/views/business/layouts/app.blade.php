...
$navOther=[
 ...
 ['route'=>'clientLogout','label'=>'Log Out','icon'=>'users','path'=>'/client/logout'],
...
<div class="flex min-h-[100dvh] w-full flex-col bg-background md:flex-row">
    <aside class="fixed inset-y-0 z-20 hidden w-64 flex-col border-r bg-card shadow-sm md:flex">
        ...
        <div class="px-4 pb-4"><div class="rounded-xl border bg-secondary/50 p-4"><h3 class="truncate text-sm font-semibold">{{ $profile['name'] }}</h3><p class="truncate text-xs text-slate-500">{{ $profile['category'] }}</p><form action="{{ route('signout') }}" method="POST">@csrf
    ...
    <main class="relative flex min-h-[100dvh] w-full flex-1 flex-col pb-[calc(5.5rem+env(safe-area-inset-bottom))] md:pl-64 md:pb-0">
        <header class="glass-nav top-0 z-30 flex flex-col gap-2 border-b px-4 py-2 md:hidden">
            ...
            <a href="{{ route('dashboard') }}" class="flex items-center gap-2"><span class="flex h-7 w-7 items-center justify-center rounded-md bg-primary font-display text-sm font-bold text-white">Q</span><span class="font-display text-sm font-bold">{{ $profile['name'] }}</span></a>
...
<div class="p-4">
    ...
    <div class="h-2 w-full overflow-hidden rounded-full bg-secondary">
        <div class="h-full rounded-full bg-{{ $color }}-500 transition-all duration-500"
            ...
            <a href="{{ route('account') }}" class="flex items-center gap-1.5 rounded-lg border bg-secondary/80 py-1.5 text-sm font-semibold"><i data-lucide="coins" class="h-4 w-4 text-accent"></i>{{ number_format($account['coins']) }}</a>
...
<div class="relative" id="notificationMenu">
   ...
   class="flex items-center gap-3 rounded-md px-3 py-2 text-sm text-slate-700 hover:bg-slate-100">
    <i data-lucide="mail-plus" class="h-4 w-4 shrink-0 text-blue-600"></i>
...
<style>
 ...
 <div class="notification-menu relative">
    <button
        ...
        class="notification-button flex items-center gap-2 rounded-lg border bg-white px-2 py-1 shadow-sm"
    >
        <img
            src="{{ asset('client/images/user.png') }}"
            loading="lazy"
            decoding="async"
            alt="{{ $profile['name'] }}"
            class="h-9 w-9 shrink-0 rounded-full border-2 border-white object-cover shadow-sm"
        >

      

        <i data-lucide="chevron-down" class="h-4 w-4 shrink-0 text-gray-500"></i>
    </button>

    <div
        id="accountDropdown"
        class="notification-dropdown absolute right-0 z-50 mt-2 hidden w-72 overflow-hidden rounded-lg border bg-white shadow-lg"
    >
        <div class="border-b px-4 py-3 text-sm font-semibold text-slate-800">
              @if(auth('clients')->check() && session()->has('switch_accounts'))
             
                <form method="get" action="{{ route('account.switch', ['type' => 'guest']) }}">
                    @csrf
                    <button
                        type="submit"
                        class="flex w-full items-center gap-3 rounded-md px-3 py-2 text-left text-sm font-medium text-gray-700 hover:bg-indigo-50 hover:text-indigo-700"
                    >
                        <i data-lucide="arrow-left-right" class="h-4 w-4 shrink-0"></i>
   ...
   class="flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-indigo-600 hover:bg-indigo-50">
    ...
    <span>Settings</span>
            ...
            <a
                ...
                class="mt-3 flex w-full items-center justify-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-xs font-semibold text-red-600 hover:bg-red-100"
            >
...
document.querySelectorAll('.notification-menu').forEach((menu) => {
    const button = menu.querySelector('.notification-button');
    const dropdown = menu.querySelector('.notification-dropdown');
    ...
    button.addEventListener('click', (event) => {
        ...
        document.querySelectorAll('.notification-menu').forEach((otherMenu) => {
            if (otherMenu === menu) return;

            otherMenu.querySelector('.notification-dropdown')?.classList.add('hidden');
            otherMenu.querySelector('.notification-button')
