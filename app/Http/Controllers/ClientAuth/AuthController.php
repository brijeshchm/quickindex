	...
	public function handleGoogleCallback(Request $request)
	{
		try {
			...
			if (!$client) {
				...
				$clientId = str_pad((string) $client->id, 4, '0', STR_PAD_LEFT);

				$client->update([
					'username' => strtoupper(substr($email, 0, 2)) . $clientId,
				]);
			}
			...
			return redirect('/business-owners')
				->with('error', 'Google login failed or this account needs linking.');
		}
	}


	public function switchAccount(Request $request, string $type)
	{
		...
		if ($type === 'clients' && Auth::guard('guest')->check()) {
			$client = Client::findOrFail($accounts['client_id']);

			if (Auth::guard('guest')->id() != $accounts['guest_id']) {
				abort(403);
			}
		...
		if ($type === 'guest' && Auth::guard('clients')->check()) {
			$guest = Guest::findOrFail($accounts['guest_id']);

			if (Auth::guard('clients')->id() != $accounts['client_id']) {
				abort(403);
			}
		...
		try {
			...
			if (!$client) {
				...
				$client = Client::create([
					...
					'last_name' => $lastName,
					'business_slug' => substr(bin2hex(random_bytes(3)), 0, 5),
					'username' => substr(bin2hex(random_bytes(3)), 0, 5),
					'client_type' => 'gold',
					'active_status' => 1,
